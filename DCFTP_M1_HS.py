    ...
    while True: ## This while Loop ends only when there is a coalscence
        ...
        for j in range(n):
            if events[n-j-1][0] == -1: ## It is a deprture in the forward chain (since it is an arrival backward in D)
                ...
                Block_in_L = Overlap_check(L, IntRange, torus_flag)
                Block_in_U = Overlap_check(U, IntRange, torus_flag)
...
print('++++++++++ dominated CFTP M1 for Gibbs HS Process (loss sys) +++++++++++++++')



for b in range(Itot):
    print('---------- DCFTP M1 Iteration: ', b+1, '---------')
